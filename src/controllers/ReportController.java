...
public class ReportController {
    ...
    // ---------------------------------------------------------------- Summary
    /** Overload kept for existing callers without filter parameters. */
    ...
    public Map<String, Double> getSummaryStats(String start, String end,
        ...
        // Revenue from payments — format: paymentId|appointmentId|customerId|amount|date
        ...
        // Counts from appointments
        // Format: appointmentId|serviceType|status|scheduledDate|totalAmount|customerId|technicianId|staffId
        for (String line : FileHandler.readData("appointments.txt")) {
            if (line.trim().isEmpty()) continue;
            String[] p = line.split("\\|");
            if (p.length < 8) continue;
            if (!isWithinRange(p[3], start, end)) continue;  // p[3] = scheduledDate
            if (!matches(p[1], svcFilter)) continue;
            if (!matches(p[6], techFilter)) continue;
            if (!matchesCategory(p[1], catFilter)) continue;
            total++;
            String status = p[2].toUpperCase();             // p[2] = status
    ...
    public Map<String, Double> getMonthlyRevenue() {
        ...
        // Only return months that have revenue — avoids chart being dominated by zero bars
    ...
    public double getAverageRating() {
        ...
        for (String line : FileHandler.readData("feedback.txt")) {
            ...
            String[] p = line.split("\\|");
            if (p.length >= 2) {
                try { sum += Integer.parseInt(p[1].trim()); count++; }
                catch (NumberFormatException ignored) {}
            }
        }
